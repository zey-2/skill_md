...
**Target outcome:** Learners can explain what an AI agent is, describe what a skill does, distinguish domain vs. workflow skills, and author a complete SKILL.md for a recurring task.

**Time estimate:** ~60 minutes total (19 slides).
...
Skills fall into two categories — often you need both:

| Type | What it encodes | Examples |
|------|----------------|----------|
| **Domain Skills** | What the agent needs to *know* | API usage patterns, framework conventions, document editing standards, technical references |
| **Workflow Skills** | How the agent should *operate* | Planning before coding, TDD enforcement, code review checklists, debugging playbooks |

**Key insight:** Skills encode *what to know* **or** *how to work*. A single skill package may combine both.
...
Before diving into anatomy, here is the template learners will fill in later:
...
Use the template from Part 3 to fill in your skill. Focus on:

1. **Name and description** — concrete, keyword-rich, under 100 tokens
2. **Trigger conditions** — at least 2 specific "when to use" cases
3. **Steps** — numbered, sequential, actionable
4. **Constraints** — at least 1 "do NOT" rule
5. **Examples** — at least 1 input → expected output pair
...
| # | Topic | Key takeaway |
|---|-------|-------------|
| 1 | Agents | LLMs that use tools in a loop to achieve goals |
| 2 | Skills | Packaged operating procedures that make agent behavior consistent |
| 2a | Domain vs. Workflow Skills | Skills encode *what to know* or *how to work* — often both |
| 3 | SKILL.md | Main skill file, kept concise via progressive disclosure |
